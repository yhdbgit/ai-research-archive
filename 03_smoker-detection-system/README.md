...
2. **경량 모델 선택 및 학습률 제어 (Trade-off: 모델 경량화 vs 탐지 정확도)**
   ...
   * **의사결정**: 산불 예방은 실시간 골든타임 확보가 핵심이므로 초당 프레임 수(FPS)를 보장할 수 있는 경량 모델인 **`SSD MobileNet V3`를 전형적인 전이 학습(Transfer Learning) 구조로 설계**했습니다. 대신 부족한 정확도는 **`Cosine Annealing LR Scheduler`**를 도입해 초반에는 빠르게 수렴하고 후반에는 미세 조정함으로써 가중치의 최적점(Global Minimum)을 안정적으로 찾도록 제어했습니다.
